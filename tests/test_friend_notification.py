...
async def test_friend_request_realtime_notification():
    ...
    token_b = jwt.encode({"sub": str(target_user_id), "exp": time.time(
    ) + 600}, settings.JWT_SECRET_KEY, algorithm="HS256")
    token_a = jwt.encode({"sub": str(sender_user_id), "exp": time.time(
    ) + 600}, settings.JWT_SECRET_KEY, algorithm="HS256")

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        from starlette.testclient import TestClient
        with TestClient(app).websocket_connect(f"/websocket/ws?token={token_b}") as websocket:
