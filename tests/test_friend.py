...
def test_search_users_success(mock_search):
    ...
    mock_search.return_value = [
        {"user_id": 2, "username": "张三丰", "avatar_url": "http://example.com/avatar2.jpg"},
    ...
    mock_search.assert_called_once_with(ANY, keyword="张三", limit=20, offset=0)
    mock_search.reset_mock()

    # 测试自定义分页
    # response = client.get("/api/friend/search?keyword=张三&page=2&size=5")
    # assert response.status_code == 200
    # mock_search.assert_called_once_with(ANY, keyword="张三", limit=5, offset=5)
...
def test_apply_friend_success(mock_create):
    ...
    response = client.post(
        "/api/friend/apply",
        json={"target_user_id": 2, "message": "交个朋友"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["code"] == 200
...
def test_apply_friend_self():
    ...
    response = client.post(
        "/api/friend/apply",
        json={"target_user_id": 1, "message": "不能加自己"}
    )
...
def test_apply_friend_repo_fails(mock_create):
    ...
    response = client.post(
        "/api/friend/apply",
        json={"target_user_id": 999, "message": "不存在"}
    )
...
def test_handle_request_accept_success(mock_handle):
    ...
    response = client.put(
        "/api/friend/handle",
        json={"request_id": 123, "action": "accepted"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["code"] == 200
...
def test_handle_request_reject_success(mock_handle):
    ...
    response = client.put(
        "/api/friend/handle",
        json={"request_id": 123, "action": "rejected"}
    )
    assert response.status_code == 200
