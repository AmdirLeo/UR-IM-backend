...
def test_get_current_user_id_success(mock_decode):
    """测试场景 1：合法的 Token，成功解析出 user_id"""
    # 模拟 jwt.decode 成功返回包含 sub 字段的字典
    mock_decode.return_value = {"sub": "1001"}
    
    # 直接像调用普通函数一样调用，不需要 await
    user_id = get_current_user_id("valid_mock_token")
    
    assert user_id == 1001
    mock_decode.assert_called_once()

@patch("api.dependencies.jwt.decode")
def test_get_current_user_id_missing_sub(mock_decode):
    ...
    with pytest.raises(BusinessException) as exc_info:
        get_current_user_id("mock_token_no_sub")
        
    assert exc_info.value.status_code == 401
    assert exc_info.value.detail == "无效的凭证载荷"

@patch("api.dependencies.jwt.decode")
def test_get_current_user_id_expired(mock_decode):
...
@patch("api.dependencies.jwt.decode")
def test_get_current_user_id_invalid(mock_decode):
